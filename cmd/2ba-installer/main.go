...
func main() {
	...
	fs.StringVar(&opts.services, "services", "", "services to configure: shell,opencode,windsurf,kimi,continue,cursor,zcode,2ba-code,claude,pi")
	...
	if sel.Claude {
		...
		// Deselecting claude on a re-run (menu or --services) must undo a
		// previous 2ba configuration: its ANTHROPIC_* vars are a global
		// knob, so a leftover settings.json would keep pointing Claude at
		// 2ba. Auto-detect (--yes) never gets here for a configured Claude:
		// the settings file keeps the config dir present, so Claude stays
		// selected and is reconfigured instead.
...
func usage() {
	...
	fmt.Fprintln(os.Stderr, "                   continue,cursor,zcode,2ba-code,claude,pi (default: all detected)")
...
func parseServices(s string) ([]string, error) {
	...
	for _, tok := range strings.FieldsFunc(s, func(r rune) bool {
		...
		switch name {
		case "shell", "opencode", "windsurf", "kimi", "continue", "cursor", "zcode", "2ba-code", "claude", "pi":
		default:
			return nil, fmt.Errorf("unknown service '%s' — valid: shell,opencode,windsurf,kimi,continue,cursor,zcode,2ba-code,claude,pi", tok)
		}
		if !seen[name] {
			seen[name] = true
			out = append(out, name)
